...
// LED
//--------------------------------------------
const byte NUM_LEDS            = 12;         // How many leds in  strip?
...
const uint32_t colors[TOTAL_PALETTES][4] = 
    ...
    { green, red   , blue  , yellow }} ;// #9 Mondriaan1
...
// LDR PHOTOCELL
//--------------------------------------------
byte     LightReducer     = 20;     // Factor to dim ledintensity with. Between 0.1 and 1 in steps of 0.05
byte     LowerBrightness  = 10;     // Lower limit of Brightness ( 0 - 255)
byte     TestLDR          = 0;      // if true LDR display is printed every second
...
void SWversion(void) 
{ 
 for (int n=0; n<80;n++) {Serial.print(F("="));} Serial.println();
 Serial.println(F("Fibonacci-klok V005"));
 Tekstprintln("Fibonacciklok december 2019  Ed Nieuwenhuys");
 Tekstprintln("\nEnter Time as: hhmm (1321) or hhmmss (132145)");
 Tekstprintln("D for Demo mode");
 Tekstprintln("I for info");
 Tekstprintln("Lnn (L5) lowest intensity (1-255)");
 Tekstprintln("Mnn (M90) light intensity (1%-250%)");
 Tekstprintln("Pn  (P1) to select a palette (0-9)");
 sprintf(sptext,"Max brightness:%3d%%",LightReducer) ;
 Tekstprintln(sptext);
 sprintf(sptext,"Min brightness:%3d bits",LowerBrightness);
 Tekstprintln(sptext);
 sprintf(sptext,"Number of LEDs: %d", NUM_LEDS);
 Tekstprintln(sptext);
 for (int n=0; n<80;n++) {Serial.print(F("="));} Serial.println();
 GetTijd(1);
}
...
{ 
 ...
 if (Key  > 32) {Serial.print(Key);}
 if (Key == 42)   // *                      // Pressing a * activates the keyboard input. 
   { 
 ...
 if (KeypadString.length()>5)          // if six numbers are entered rework this to a time hhmmss
   {    
    ...
    KeypadString = "";
    Serial.println(F("time reworked"));
   }
...
{                                                                                                       
 if (SecPulse) 
 {
  ...
  if(print)
  {
   ...
   sprintf(sptext," Temp:%2ldC ",(long) get3231Temp());             Tekstprintln(sptext);
   Print_tijd(); 
  }
