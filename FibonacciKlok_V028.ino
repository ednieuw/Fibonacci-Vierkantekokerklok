/* =====================================
 ...
 V023 Used for two Fibonacci Stick clock 50cm. One for Addy & Hans
      Working version for Arduino Nano
 V024 Added One wire 3x1 membrane button ONEWIREKEYPAD3x1 
 V025 Store 24 bytes of the last 24 hour LDR readings to DS3231 NVRAM
      Corrected rotary encoder bug in processing key press looping forever
...
const int NUM_LEDS = 13;       // How many leds in fibonacci clock? Chrono clock has 12 LEDs!  --->  Update LED positions in  setPixel() (at +/- line 920) TOO!  
...
char VERSION[] = "FibonacciKlok_V028";         // Version of this coding
...
byte  Dimfactor                = 128;                               // Used for SK6812 and FABLED libraries to retain brightness
                         #ifdef LED6812 
                            #ifdef NEOPIXEL   
...
RGBW  StrippixelDimmed[NUM_LEDS+1];// = {0};                             // Array holding the pixel send to the pixel after dimming 
                             ...
                             #endif //LIBSK6812
                             #ifdef FABLED
sk6812b<D,LED_PIN>  strip;                                          // Initialyse FABLED library (Use sk6812<D,LED_PIN>  strip;  if RGB is wrong    
grbw  Strippixel[NUM_LEDS+1] ;//= {0};                                   // Array holding the original pixel data
grbw  StrippixelDimmed[NUM_LEDS+1];// = {0};                             // Array holding the pixel send to the pixel after dimming 
                             #endif //FABLED
                         #endif //LED6812  
                         #ifdef LED2812
                            #ifdef NEOPIXEL  
Adafruit_NeoPixel strip = Adafruit_NeoPixel(NUM_LEDS, LED_PIN, NEO_GRB + NEO_KHZ800);  //NEO_RGB NEO_GRB
                            #endif //NEOPIXEL
                            #ifdef FABLED
ws2812b<D,LED_PIN>  strip;                                          // Initialyse FABLED library (Use ws2812<D,LED_PIN>  strip;  if RGB is wrong    
grb  Strippixel[NUM_LEDS+1];// = {};
grb  StrippixelDimmed[NUM_LEDS+1];// = {};                              // Array holding the pixel send to the pixel after dimming 
                            #endif //FABLED
                         #endif //LED2812
//--------------------------------------------
// COLOURS
...
void setup()         // initialise the hardware // initialize the appropriate pins as outputs:
{
 ...
 Tekstprintln("3*1 keypad enabled");  
 ...
 Tekstprintln("HC-12 time receiver");
                          #endif //HC12MOD 
...
void Reset(void)
{
 ...
 Mem.LowerBrightness  = 10;                                          // Lower limit of Brightness ( 0 - 255)
 ...
 for(int i=0; i<12; i++)
  {
   ...
   if(i == Secsegment)  Bit = 4;                                   // If second use the fifth colour to display 
...
*/
  ...
  switch(pixel)  
  {
   ...
   case 2:      ColorLeds("", 2, 3,kleur); break;
   case 3:      ColorLeds("", 4, 6,kleur); break;
   case 4:      ColorLeds("", 7,13,kleur); break; 
  }
}
...
//--------------------------------------------
//void ShowLeds(unsigned int DimFactor)
void ShowLeds(void)
{
                       #ifdef FABLED
 for(unsigned int i = 0; i  < NUM_LEDS; i++)                              // Fill the Array with dimmed values
  { 
    StrippixelDimmed[i].r = Strippixel[i].r * Dimfactor / 255;
    StrippixelDimmed[i].g = Strippixel[i].g * Dimfactor / 255;
    StrippixelDimmed[i].b = Strippixel[i].b * Dimfactor / 255;
                     #ifdef LED6812
    StrippixelDimmed[i].w = Strippixel[i].w * Dimfactor / 255;
                     #endif  // LED6812
  } 
                       #endif  // FABLED   
                     #ifdef LIBSK6812
 for(unsigned int i = 0; i  < NUM_LEDS; i++)                              // Send the Array with dimmed values
     LED.set_rgbw(i, {StrippixelDimmed[i].r, StrippixelDimmed[i].g, StrippixelDimmed[i].w, StrippixelDimmed[i].w});
 LED.sync();
                     #endif // LIBSK6812
                     #ifdef FABLED
 strip.sendPixels(NUM_LEDS,StrippixelDimmed);    
                     #endif  // FABLED                
                     #ifdef NEOPIXEL  
 strip.show();
                     #endif //NEOPIXEL
...
{
 Dimfactor = Bright;                           // place it in a global variable Dimfactor so it can be use in ShowLeds()
                     #ifdef NEOPIXEL  
...
{
                   #ifdef FABLED   
  for(int i = FirstLed; i < FirstLed + NoofLEDs; i++)
  {
    Strippixel[i].r = Cred(RGBWColor);
    Strippixel[i].g = Cgreen(RGBWColor);
    Strippixel[i].b = Cblue(RGBWColor);
                   #ifdef LED6812  
    Strippixel[i].w = Cwhite(RGBWColor);
                   #endif  // LED6812    
  }
                   #endif  // FABLED   
                   #ifdef NEOPIXEL    
...
{                                                                                                       
  int LDR_read = (4 * Previous_LDR_read + 3*analogRead(PhotoCellPin)) / 5;                  // Read lightsensor 
//  int Temp;
  ...
  if(print)
  {
   ...
   sprintf(sptext,"=%2d%%",(int)(BrightnessCalcFromLDR/2.55));      Tekstprint(sptext);
